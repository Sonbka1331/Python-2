...
def main_with_pool():
    ...
    with Pool() as pool:
        character_urls = [base_url + str(i) + '/' for i in range(1, 21)]
        pool.map(download_character_data, character_urls)

    end_time = time.time()
    execution_time = end_time - start_time
    print(f"Время выполнения программы с использованием Pool: {execution_time} сек")


def main_with_threadpool():
    create_table()
    ...
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"Время выполнения программы с использованием ThreadPool: {execution_time} сек")


if __name__ == '__main__':
    main_with_pool()
    main_with_threadpool()
